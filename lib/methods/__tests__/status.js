...
describe('status', () => {
  ...
  it('new file => new', async () => {
    ...
    const actual = await repo.status();
    expect(actual).toEqual([{ status: 'new', path: 'README.md' }]);
  });
  it('new file in staging => added', async () => {
    ...
    await repo.add();
    const actual = await repo.status();
    expect(actual).toEqual([{ status: 'added', path: 'README.md' }]);
  });
  it('existing file modified => updated', async () => {
    await repo.init();
    await repo.newFile('README.md');
    await repo.add();
    await repo.commit('save readme');
    await repo.write('updated readme', 'README.md');
    const actual = await repo.status();
    expect(actual).toEqual([{ status: 'modified', path: 'README.md' }]);
  });
