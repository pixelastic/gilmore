import { emptyDirectory, tmpDirectory } from 'firost';
...
describe('readFile', () => {
  ...
  it('should handle filepaths with spaces at a specific commit', async () => {
    await repo.init();
    await repo.writeFile('space content', 'path with spaces/my file.txt');
    const commit = await repo.commitAll();

    const actual = await repo.readFile('path with spaces/my file.txt', commit);
    expect(actual).toBe('space content');
  });
